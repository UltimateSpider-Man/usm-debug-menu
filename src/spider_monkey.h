...
inline void render()
{
...

    {
        ...
        if (os_developer_options::instance()->get_flag(mString { "SHOW_WATERMARK_VELOCITY" })) {
            g_game_ptr()->show_max_velocity();
        }
        if (os_developer_options::instance()->get_flag(mString { "WAIT_FOR_LINK" })) {
            g_game_ptr()->advance_state_wait_link(10.0f);
        }
        ...
        if (os_developer_options::instance()->get_flag(mString { "SHOW_FPS" })) {
            ...
            auto v39 = (((g_game_ptr()->field_278 - g_game_ptr()->field_27C) - g_game_ptr()->field_280) >= 0.000099999997
